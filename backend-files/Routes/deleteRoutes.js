...
router.delete("/origem/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM origem WHERE id = $1 RETURNING *",
      [id],
    );
...
router.delete("/intencao-compra/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM intencao_compra WHERE id = $1 RETURNING *",
      [id],
    );
    if (result.rowCount === 0) {
      return res
        .status(404)
        .json({ error: "Intenção de compra não encontrada" });
    }
    res
      .status(200)
      .json({ message: "Intenção de compra deletada com sucesso!" });
  } catch (error) {
...
router.delete("/veiculos/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM veiculo_interesse WHERE id = $1 RETURNING *",
      [id],
    );
    if (result.rowCount === 0) {
...
// Deletar Carro de interesse
router.delete("/carro/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query(
      "DELETE FROM carros WHERE id_carro = $1 RETURNING *",
      [id],
    );
    if (result.rowCount === 0) {
...
// Deletar Motivo de interesse
router.delete("/motivo/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query(
      "DELETE FROM motivos_saida WHERE id_motivo = $1 RETURNING *",
      [id],
    );
    if (result.rowCount === 0) {
      return res.status(404).json({ error: "Carro não encontrado" });
