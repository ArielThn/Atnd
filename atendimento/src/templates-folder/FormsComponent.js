  ...
  const addNewOrigem = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/origem', {
        ...
        body: JSON.stringify({ descricao: newOrigem }),
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const addNewIntencao = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/intencao-compra', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ descricao: newIntencao }),
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const addNewVeiculo = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/veiculos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ descricao: newVeiculo }),
      });
      if (response.ok) {
        const data = await response.json();
          ...
          <form class="space-y-6" onSubmit={handleSubmit}>
            ...
            <div class="space-y-4 border-b-2 border-[#001e50] pb-6">
              ...
              <div class="flex flex-col space-y-4">
                {origins.map((origin) => (
                  <label class="flex items-center justify-between px-4" key={origin.id}>
                    <span class="text-gray-800">{origin.descricao}</span>
                    <input
                      type="radio"
                      name="origin"
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.value === 'Aniversariante') {
                          resetForm();
                        }
                      }}
                  ...
                  <div class="flex flex-col space-y-4">
                    {intentions.map((intention) => (
                      <label class="flex items-center justify-between px-4" key={intention.id}>
                        <span class="text-gray-800">{intention.descricao}</span>
                        <input
                          type="radio"
                          name="intention"
